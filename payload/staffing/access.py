...
def _preferred_wedding(user):
    """Return the user's selected workspace, or the latest accessible workspace."""
    if not getattr(user, "is_authenticated", False):
        return None
...
def get_wedding_for_user(user, permission=None):
    ...
    A permission check never silently jumps to a different wedding. This is important
    for tenant isolation: if the selected workspace does not grant the requested
    permission, callers receive None instead of data from another wedding.
    """
